...
class SamplingConfig:
    ...
    use_dec_sc: bool = False  # decode-branch SC signal per step; +1 fwd pass/step, no retraining
    dec_sc_alpha: float = 1.0        # A3: blend weight for dec_sc; 1.0=full replace, <1.0=interpolate with original x̂
    dec_sc_use_prev_sc: bool = False  # A4: use x̂_{k-1} (not zeros) as SC input inside decode branch
    dec_sc_start_frac: float = 0.0   # A1: fraction of total steps at which to begin applying dec_sc (0.0=all steps)
...
class Config:
    ...
    kd_gate_k: float = 10.0      # sigmoid sharpness for ω(t) gate
    kd_gate_low: float = 0.25    # gate rising edge  (plateau default: 0.25)
    kd_gate_high: float = 0.95   # gate falling edge (plateau default: 0.95; commit-release: 0.60)
    kd_position_mask: bool = False  # Idea B: weight KD by confidence × wrong-committed per position
    ...
    log_freq: int = 100
    eval_freq: int = 10
    save_freq: float = 100.0  # Can be fractional (e.g., 0.1 for saving every 0.1 epoch)

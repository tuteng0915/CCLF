...
| Pipeline mainly fails because it queries a shared average clock | EXP-70 true-local-clock oracle is no better, while mixed-state error is 3--4x clock error; EXP-80 conditional local+refine8 has prompt PPL `1915` versus Standard-136 `183` | heterogeneous context is the dominant failure; real prefix conditioning does not rescue the discrete Pipeline schedule |
| repeated synchronized soft prefix leadership is an efficient sampler | EXP-71 correct-content arms all lose to compute-matched ODE-64; EXP-80 conditional soft LTR/random have prompt PPL `512/506` versus Standard-64 `267`, with random again better than LTR | keep the shuffled-content contrast as mechanism evidence only |
...
| predicted-clean input replacement canonicalizes heterogeneous Pipeline context | EXP-75 lowers PPL partially but leaves vector error unchanged/worse; EXP-80 conditional canonical+refine8 has prompt PPL `765`, ROUGE-L `.052`, and degeneration `.219` versus Standard-136 `183/.082/.094` | correct content is useful, but simple replacement is not a shared dynamical coordinate system even with a real prefix |
...
| late clock alignment plus native `x0` conditioning recovers parallel conditional quality | EXP-79 fixed-prefix P1: late m24/m28 prompt-conditioned PPL `419.0/420.2`, only Semi-AR level `421.2` and far worse than Parallel-32/60 `252.8/134.5`; ROUGE-L and A-to-B boundary PPL also favor parallel | conditional evidence rejects this implementation; do not expand it without a materially different algorithm |

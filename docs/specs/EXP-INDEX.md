...
| **EXP-63** | **READY / P0** | ELF baseline corrected fine-tuning control | clean-`x0`, `t=1` stop-gradient decoder teacher + smooth JAX gate versus matched continued training | 先通过 unit test 和 2-step smoke，再跑 matched 2k control/full-KD；只有定性生成也健康才重开 temporal windows；见 EXP-63-spec.md |

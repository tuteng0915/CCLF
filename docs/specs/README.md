...
| [EXP-93](EXP-93-spec.md) | **STAGE 2 NEGATIVE / ORACLE GAP UNPREDICTABLE** | Is random anchoring close to subset-optimal, and can trigger-time features predict better masks? | best-of-16 has large headroom, but grouped OOF ranking stays near chance on two banks; close the current selector branch |
...
| [EXP-95](EXP-95-spec.md) | **STAGE 2 POSITIVE / FORMAL REPLICATION RUNNING** | Is there a Plaid temporary-anchor setting that improves PPL without the ELF diversity trade-off? | four complete-control cells pass on seed 42; two frozen early single-step settings receive seeds 123/456 |

...
| [EXP-93](EXP-93-spec.md) | **STAGE 2 CLOSED / REPLICATED ORACLE GAP, CURRENT SELECTORS FAIL** | Is random anchoring close to subset-optimal, and can trigger-time features predict better masks? | two banks show `37--38%` oracle C-PPL headroom, but static, 8-step lookahead, and additive causal-graph selectors fail independent validation |
...
| [EXP-95](EXP-95-spec.md) | **DONE / THREE-SEED PLAID METHOD POSITIVE** | Is there a Plaid temporary-anchor setting that improves PPL without the ELF diversity trade-off? | early one-step 75% confidence anchors reduce C-PPL `110.39 -> 80.28`, improve mean D1, and remain highly revisable; D2 falls slightly |
...
| [EXP-107](EXP-107-spec.md) | **IMPLEMENTED / PILOT PENDING** | Can pathwise response select anchor versus control directly? | require significant C-NLL improvement over fixed anchoring with complete quality gate |
